...
target_ip="$(virsh --quiet -c qemu:///system domifaddr test-target-fedora | tail -n1 | awk '{print $4}' | cut -d'/' -f1)"
...
if [ "$1" = "shell" ]; then
	...
	$run_cmd "openwrt-iscsi-target-ramdisk/test/fedora/run-in-vm.sh"
fi