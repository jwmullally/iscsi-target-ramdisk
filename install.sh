...
uninstall_previous() {
    if [ -f "/usr/local/sbin/uninstall-iscsi-target-ramdisk.sh" ]; then
        echo "Previous version detected, uninstalling before continuing..."
        /usr/local/sbin/uninstall-iscsi-target-ramdisk.sh
...
enable_dracut_iscsi() {
    ...
    install -m 0644 -T src/dracut.conf /etc/dracut.conf.d/90-iscsi-target-ramdisk.conf
...
install_boot_entry() {
    echo "Installing iSCSI Target Ramdisk boot menu entry"
    if [ "$HAS_BLS" = "1" -a "$HAS_OSTREE" = "0" ]; then
        install -m 0644 -T src/bootloaderspec-entry.conf /boot/loader/entries/iscsi-target-ramdisk.conf
    else
        install -m 0755 -T src/grub-entry.sh /etc/grub.d/42_iscsi-target-ramdisk
        if [ "$HAS_GRUB1" = "1" ]; then
...
preserve_kernel_cmdline
set_bootif_unmanaged
install -m 0755 -T uninstall.sh /usr/local/sbin/uninstall-iscsi-target-ramdisk.sh
./update.sh
